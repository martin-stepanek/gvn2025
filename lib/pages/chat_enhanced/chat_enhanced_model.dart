...
class ChatEnhancedModel extends FlutterFlowModel<ChatEnhancedWidget> {
  ...
  Future submitPrompt(BuildContext context) async {
    ...
    awaitingReply = true;
    logFirebaseEvent('submitPrompt_backend_call');
    gptCheck = await AzureGroup.aZcoachThreePointFiveCall.call(
  ...
  Future selectGenderUnset(BuildContext context) async {
    logFirebaseEvent('selectGenderUnset_backend_call');
    ...
    logFirebaseEvent('selectGenderUnset_update_page_state');
    promptSuggestions = [];
    progress = ProcessStep.step_05b_gender_selection;
    logFirebaseEvent('selectGenderUnset_backend_call');

    await widget!.chat!.reference.update(createChatsRecordData(
      step: ProcessStep.step_06_age_selection,
    ));
  }

  Future selectAgeUnset(BuildContext context) async {
    logFirebaseEvent('selectAgeUnset_backend_call');
    ...
    logFirebaseEvent('selectAgeUnset_update_page_state');
    promptSuggestions = [];
    progress = ProcessStep.step_06_age_selection;
    logFirebaseEvent('selectAgeUnset_backend_call');
  ...
  Future selectGenderSet(BuildContext context) async {
    ...
    await MessagesRecord.createDoc(widget!.chat!.reference)
      ...
      firstMessage: false,
      message:
          'Using your previous gender input (${currentUserDocument?.gender?.name}). Create a new account to reset it.',
      user: User.coach,
      userRef: currentUserReference,
      include: true,
      secondsToGenerate: 0.0,
    ...
    logFirebaseEvent('selectGenderSet_update_page_state');
    promptSuggestions = [];
    progress = ProcessStep.step_05_gender;
    logFirebaseEvent('selectGenderSet_backend_call');

    await widget!.chat!.reference.update(createChatsRecordData(
      step: ProcessStep.step_05_gender,
    ));
  }

  Future selectAgeSet(BuildContext context) async {
    logFirebaseEvent('selectAgeSet_backend_call');

    await MessagesRecord.createDoc(widget!.chat!.reference)
        .set(createMessagesRecordData(
      timestamp: getCurrentTimestamp,
      firstMessage: false,
      message: 'Using your previous age input (${formatNumber(
        valueOrDefault(currentUserDocument?.age, 0.0),
        formatType: FormatType.custom,
        format: '###.#',
        locale: '',
      )} years). Create a new account to reset it.',
      user: User.coach,
      userRef: currentUserReference,
      include: true,
      secondsToGenerate: 0.0,
    ...
    logFirebaseEvent('selectAgeSet_update_page_state');
    promptSuggestions = [];
    progress = ProcessStep.step_06_age;
    logFirebaseEvent('selectAgeSet_backend_call');

    await widget!.chat!.reference.update(createChatsRecordData(
      step: ProcessStep.step_06_age,
    ));
  }
